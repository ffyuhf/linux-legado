...
export const useBookStore = defineStore('book', {
    state: () => {
        return {
            ...
            shelf: [],
            catalog: [],
            readingBook: { chapterPos: 0, chapterIndex: 0 },
    ...
    actions: {
        ...
        async loadBookShelf() {
            const fetchBookshellf_promise = API.getBookShelf().then(resp => {
                ...
                if (isSuccess === true) {
                    if (this.shelf.length !== data.length &&
                        this.shelf.length > 0 &&
                        data.length > 0) {
        ...
        /** 从后端加载书籍目录，优先返回内存缓存 */
        async loadWebCatalog(book) {
            const { bookUrl, name, chapterIndex } = book;
            const fetchChapterList_promise = API.getChapterList(bookUrl).then(res => {
                const { isSuccess, data, errorMsg } = res.data;
                if (isSuccess === false) {
                    ElMessage.error(errorMsg);
                    throw new Error();
                }
                if (bookUrl === this.readingBook.bookUrl &&
        ...
        async loadWebConfig() {
            if (webReadConfigLoadedDate === undefined) {
                ...
                console.log(`${this.$id}.loadWebConfig: ${webReadConfigLoadedDate.toLocaleString()}成功加载阅读配置`);
                return this.setConfig(_config);
            }
            console.log(`${this.$id}.loadWebConfig: 已于${webReadConfigLoadedDate.toLocaleString()}成功加载`);
        },
        setConfig(config) {
        ...
        async setSearchBooks(books) {
            books.forEach(book => {
                const isSeachBook = this.shelf.every(item => item.bookUrl !== book.bookUrl);
                if (isSeachBook === true) {
        ...
        async saveBookProgress() {
            ...
            if (findIndex > -1) {
                this.shelf[findIndex] = Object.assign({}, shelfRaw[findIndex], this.bookProgress);
            }
