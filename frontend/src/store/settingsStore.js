...
export const useSettingsStore = defineStore('settings', () => {
    ...
    const saveAllSettings = async (changes) => {
        ...
        try {
            ...
            if (data.isSuccess) {
                ...
                // 更新本地缓存
                if (changes.app)
                    appSettings.value = { ...appSettings.value, ...changes.app };
                if (changes.read)
                    readSettings.value = { ...readSettings.value, ...changes.read };
                if (changes.theme)
                    themeSettings.value = { ...themeSettings.value, ...changes.theme };
            }
            else {
    ...
    const saveAppSettings = async (settings) => {
        ...
        try {
            ...
            ElMessage.error('保存应用设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
     ...
     */
    const saveReadSettings = async (settings) => {
        saving.value = true;
        try {
            ...
            ElMessage.error('保存阅读设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const saveThemeSettings = async (settings) => {
        ...
        try {
            ...
            ElMessage.error('保存主题设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const resetAppSettings = async () => {
        ...
        try {
            ...
            ElMessage.error('重置应用设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const resetReadSettings = async () => {
        ...
        try {
            ...
            ElMessage.error('重置阅读设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const resetThemeSettings = async () => {
        ...
        try {
            ...
            ElMessage.error('重置主题设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const resetAllSettings = async () => {
        ...
        try {
            ...
            ElMessage.error('重置全部设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
     ...
     * @returns 设置值
     */
    const getSetting = async (key, category = 'app', defaultValue) => {
        try {
            ...
            ElMessage.error('获取设置失败: ' + e.message);
        }
        return undefined;
    };
    /**
     ...
     * @param category 配置类别: app/read/theme
     */
    const saveSetting = async (key, value, category = 'app') => {
        saving.value = true;
        try {
     ...
     * @param category 配置类别: app/read/theme
     */
    const deleteSetting = async (key, category = 'app') => {
        saving.value = true;
        try {
            ...
            ElMessage.error('删除设置失败: ' + e.message);
        }
        finally {
            saving.value = false;
        }
    ...
    const saveSearchHistory = async (word) => {
        ...
        try {
            ...
            ElMessage.error('保存搜索历史失败: ' + e.message);
        }
    };
    /**
     * 清除全部搜索历史
