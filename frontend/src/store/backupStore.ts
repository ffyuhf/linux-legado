...
export const useBackupStore = defineStore('backup', () => {
  ...
  const createBackup = async (options: BackupOptions = {}) => {
    ...
    try {
      ...
      ElMessage.error('创建备份失败: ' + (e as Error).message)
    } finally {
   ...
   * @returns 恢复统计结果
   */
  const restoreBackup = async (file: File): Promise<RestoreResult | null> => {
    restoring.value = true
    try {
      const { data } = await API.restore(file)
      if (data.isSuccess && data.data) {
        const result = data.data as RestoreResult
        ElMessage.success(
          `恢复成功：书籍 ${result.books} 个，源 ${result.sources} 个，配置 ${result.configs} 项`,
        )
        return result
      } else {
      ...
      ElMessage.error('恢复备份失败: ' + (e as Error).message)
    } finally {
  ...
  const exportSources = async () => {
    try {
      ...
      ElMessage.error('导出书源失败: ' + (e as Error).message)
    }
  }
  ...
  const importSources = async (
    ...
    try {
      ...
      if (data.isSuccess) {
        const count = data.data as number
      ...
      ElMessage.error('导入书源失败: ' + (e as Error).message)
    }
    return 0
  }
   ...
   * @returns 导入数量
   */
  const importSourcesFromFile = async (file: File): Promise<number> => {
    try {
      ...
      ElMessage.error('解析书源文件失败: ' + (e as Error).message)
      return 0
    }
  }
