...
import { emptyBookSource, emptyRssSource, getSourceUniqueKey, convertSourcesToMap, } from '@utils/souce';
const isBookSource = /bookSource/i.test(location.href);
const emptySource = isBookSource ? emptyBookSource : emptyRssSource;
export const useSourceStore = defineStore('source', {
    state: () => {
        return {
            bookSources: shallowRef([]), // 临时存放所有书源,
            rssSources: shallowRef([]), // 临时存放所有订阅源
    ...
    getters: {
        ...
        currentSourceUrl: state => isBookSource
            ? state.currentSource.bookSourceUrl
            : state.currentSource.sourceUrl,
        searchKey: (state) => isBookSource
    ...
    actions: {
        ...
        saveSources(data) {
            if (isBookSource) {
                this.bookSources = markRaw(data);
            }
            else {
                this.rssSources = markRaw(data);
            }
        },
        //批量推送
        ...
        //保存当前编辑源
        saveCurrentSource() {
            const source = this.currentSource, map = this.sourcesMap;
