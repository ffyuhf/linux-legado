...
export const useBookGroupStore = defineStore('bookGroup', () => {
  ...
  const loadGroups = async () => {
    ...
    try {
      ...
      if (data.isSuccess && data.data) {
        groups.value = data.data as unknown as BookGroup[]
      } else {
        ElMessage.error(data.errorMsg || '加载分组失败')
      }
    } catch (e) {
      ElMessage.error('加载分组失败: ' + (e as Error).message)
    } finally {
      loading.value = false
    }
  }
  ...
  /** 保存分组（新增或更新） */
  const saveGroup = async (group: Partial<BookGroup>) => {
    saving.value = true
    try {
      const { data } = await API.saveBookGroup(group as Record<string, unknown>)
      if (data.isSuccess) {
      ...
      ElMessage.error('保存分组失败: ' + (e as Error).message)
    } finally {
      saving.value = false
    }
  }
  ...
  /** 更新分组顺序 */
  const updateOrder = async (orders: number[]) => {
    try {
      ...
      ElMessage.error('更新分组顺序失败: ' + (e as Error).message)
    }
  }

  /** 更新书籍所属分组 */
  const updateBookGroup = async (bookUrls: string[], groupId: number) => {
    saving.value = true
    try {
      ...
      ElMessage.error('更新书籍分组失败: ' + (e as Error).message)
    } finally {
      saving.value = false
    }
  }
