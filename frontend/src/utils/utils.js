...
export const isNullOrBlank = (string) => string == null ||
    string.length === 0 ||
    /^\s+$/.test(string);
export const isLegadoUrl = (/** @type {string} */ url) => /,\s*\{/.test(url) ||
    !(url.startsWith('http') ||
        url.startsWith('data:') ||
        url.startsWith('blob:'));
/**
 ...
 */
export const validatorHttpUrl = (http_url, allowedProtocols = ['https:', 'http:']) => {
    try {
        const url = new URL(http_url);
        const { protocol } = url;
        if (!allowedProtocols.includes(protocol))
            throw new Error(`Expected protocol ${allowedProtocols.join('/')}, but ${protocol}`);
        return true;
    }
    catch {
        return false;
 ...
 * 懒加载正则
 */
export const lazyRegex = (pattern, flags) => {
    let instance = null;
    return () => {
        if (!instance) {
