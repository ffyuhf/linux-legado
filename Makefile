...
# 2026-06-17 codewhale - package-appimage 显式传 ARCH=$(APP_ARCH) 给 appimagetool,
#                       修复 AppRun 为脚本(非ELF二进制)时无法自动推断架构导致 CI 失败
...
APP_VERSION  := 1.0.0
APP_MAINT    := nmb
# AppImage 目标架构 (appimagetool 无法从脚本型 AppRun 推断架构, 故需显式指定);
# 默认 x86_64, 可用 `APP_ARCH=aarch64 make package-appimage` 覆盖以构建 arm64 产物
APP_ARCH     ?= x86_64
...
package-appimage: backend-jar icon ## 构建 .AppImage 单文件 (所有发行版, 需 appimagetool)
	...
	@mkdir -p $(APPDIST_DIR)
	APP_NAME=$(APP_NAME) APP_VERSION=$(APP_VERSION) ARCH=$(APP_ARCH) appimagetool \
		$(APPDIR_ROOT) $(APPDIST_DIR)/$(APP_NAME)-$(APP_VERSION)-$(APP_ARCH).AppImage
	@echo "==> .AppImage 构建完成: $(APPDIST_DIR)/$(APP_NAME)-$(APP_VERSION)-$(APP_ARCH).AppImage"
